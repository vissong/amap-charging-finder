...
`AMAP_MAX_QPS` 默认值为 `3`，只允许设置为 `1` 至 `3`，可降低但不能突破当前 Key 的安全上限。服务端会让所有高德上游请求共享同一个内存 FIFO 队列，相邻请求至少间隔 `ceil(1000 / AMAP_MAX_QPS)` 毫秒。队列最多等待 30 个请求，单个请求最多等待 10 秒；浏览器断开后会取消对应排队请求。队列已满或等待超时时，接口返回 `503 AMAP_QUEUE_BUSY`，而不是误报高德故障。高德返回 QPS 错误时，整个队列统一冷却 1 秒并重试当前请求一次。个人认证开发者的基础搜索服务公开配额为 3 QPS，但实际 Key 配额应以高德控制台“流量分析 → 配额管理”为准：<https://lbs.amap.com/pages/base_service_price>。
...
限流队列只在当前 Node 进程内全局共享，适合仓库默认的单应用容器部署。如果扩展为多个应用实例，实例之间无法在不使用 Redis 等外部协调服务的情况下共享队列，需要把每个实例的 `AMAP_MAX_QPS` 设置为总配额除以实例数后的保守整数值。
